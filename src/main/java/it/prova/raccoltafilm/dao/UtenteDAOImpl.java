...
public class UtenteDAOImpl implements UtenteDAO {
	...
	public List<Utente> findByExample(Utente example) throws Exception {
		...
		StringBuilder queryBuilder = new StringBuilder("select f from Utente f where f.id = f.id ");

		if (StringUtils.isNotEmpty(example.getUsername())) {
			whereClauses.add(" f.username  like :username ");
			paramaterMap.put("username", "%" + example.getUsername() + "%");
		}
		if (StringUtils.isNotEmpty(example.getNome())) {
			whereClauses.add(" f.nome like :nome ");
			paramaterMap.put("nome", "%" + example.getNome() + "%");
		}
		if (StringUtils.isNotEmpty(example.getCognome())) {
			whereClauses.add(" f.cognome like :cognome ");
			paramaterMap.put("cognome", "%" + example.getCognome() + "%");
		}
		if (example.getDateCreated() != null) {
			whereClauses.add("f.dateCreated >= :dateCreated ");
